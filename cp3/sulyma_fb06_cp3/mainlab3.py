...
def most_common_bigrams(text):
    ...
    for i in range(len(text) - 1):
        ...
        if bigram not in fregs_birgam:
            fregs_birgam[bigram] = 0
        fregs_birgam[bigram] = fregs_birgam[bigram] + 1
    final_bigrams = list(sorted(fregs_birgam.items(), key=lambda item: item[1], reverse=True))[:5]
    sort_bigram = []
