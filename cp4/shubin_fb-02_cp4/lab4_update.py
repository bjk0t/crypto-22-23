...
def gcd(a, b):  # Алгоритм Евкліда. повертає b=НСД(a, b)
    if a == 0:
        return b
    else:
        b = gcd(b % a, a)
...
def rsa_send_key(k, sender_publickey, sender_secretkey, receiver_publickey):
    ...
    s = rsa_sign(k, sender_secretkey, sender_publickey)
    # k1 = gmpy2.powmod(k, e1, n1)
    k1 = rsa_encrypt(k, receiver_publickey)
    # s1 = gmpy2.powmod(s, e1, n1)
    s1 = rsa_encrypt(s['signature'], receiver_publickey)
    return {'k1': k1, 's1': s1}
...
def rsa_receive_key(k1, s1, receiver_publickey, receiver_secretkey, sender_publickey):
    ...
    k = rsa_decrypt(k1, receiver_secretkey, receiver_publickey)
    # s = gmpy2.powmod(s1, d1, n1)
    s = rsa_decrypt(s1, receiver_secretkey, receiver_publickey)
    ...
    authentication = rsa_verify(k, s, sender_publickey)
